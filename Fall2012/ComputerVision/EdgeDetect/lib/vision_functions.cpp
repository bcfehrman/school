...
void createGausianKernal( Mat &kernDst, double standardDeviation)
{
	...
	for(int i = 0; i < height; i++)
	{
      ...
      if( fftKern )
      {
         for(int j = 0; j < width; j++)
         {
            ...
            kernDst.at<float>(i,j) = kernelVal;
   ...
   else
   {
      for(int i = 0; i < height; i++)
      {
         for( int j = 0; j < width; j++ )
         {
            kernDst.at<float>(i,j) = kernDst.at<float>(i,j) / preNormSum;
            normSum += kernDst.at<float>(i,j);
         }
      } 
   }
}
