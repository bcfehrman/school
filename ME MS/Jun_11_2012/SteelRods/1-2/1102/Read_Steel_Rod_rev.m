function [] = Read_Steel_Rod_rev(filePrefix, firstIteration, secondIteration)
...
temp = load(strcat( filePrefix, num2str(firstIteration), '_Signals.lvm'));
temp2 = temp(:,3);
defect(:,1) = temp2(1:N);

temp = load(strcat( filePrefix, num2str(secondIteration), '_Signals.lvm'));
temp2 = temp(:,3);
defect(:,2) = temp2(1:N);
...
figure(figure_count)
plot(t_theory, vlongchirp1,'-r', t_theory, defect(:, 1),'--b', t_theory2, vlongchirp2,'-r', t_theory2, defect(:, 2),'--b','LineWidth',2)
%title({'Theoretical and Experimental Defect Responses Superimposed', 'Intial Phase and Time Reversal Phase {First Iteration}', '(actual time delay between phases not shown due to magnitude)'})
...
title('Defect response for rods 1 and 2')
legend('Theoretical Defect Response', 'Experimental Defect Response','location','best')
figure_count = figure_count + 1;
filename = strcat(filePrefix, 'th_exp');
